...
admission webhook returns a warning:

```text
no volume is mounted at the data path "/var/lib/clickhouse", which may lead to
data loss if the cluster is restarted
```
...
it by hand. It renders a configuration file (`10-storage-jbod.yaml`) that registers
every additional disk and adds it to the built-in `default` storage policy:

```yaml
storage_configuration:
  disks:
    disk1:
      path: /var/lib/clickhouse/disks/disk1/
    disk2:
      path: /var/lib/clickhouse/disks/disk2/
  policies:
    default:
      volumes:
        main:
...
- [Scaling clusters](/products/kubernetes-operator/guides/scaling) — how PVCs are created and cleaned up as replicas and shards change.
