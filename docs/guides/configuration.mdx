...
| `KeeperCluster` | `replicas: 1` | `maxUnavailable: 1` — single-replica clusters cannot use `replicas/2 = 0`, which would block every voluntary disruption (introduced in [#208](https://github.com/ClickHouse/clickhouse-operator/pull/208)) |
