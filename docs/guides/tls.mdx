...
description: 'End-to-end walkthrough for securing a ClickHouse cluster with TLS using cert-manager, including client connections and Keeper encryption.'
doc_type: 'guide'
---
...
`/etc/clickhouse-server/tls/` and wires them into ClickHouse's `openSSL` config.
...
clients address the cluster — the per-replica headless service and the
...
- **Mounts the Secret** at `/etc/clickhouse-server/tls/` and generates the
  ...
  `disableProtocols: sslv2,sslv3`, and `preferServerCiphers: true`.
...
- **Removes the insecure ports** `9000` (native) and `8123` (HTTP) — only the TLS
  variants remain. Plaintext clients can no longer connect.
...
With `required: true`, clients must use the secure ports and trust the CA.
...
curl --cacert /path/to/ca.crt \
  "https://<cluster-name>.<namespace>.svc.cluster.local:8443/?query=SELECT%201"
```
...
**Confirm the secure ports are live on the Service:**

```bash
kubectl -n <namespace> get svc <cluster-name> \
...
|---|---|
| Cluster stuck reconciling after enabling TLS | `serverCertSecret` missing, or the Secret lacks `tls.crt`/`tls.key`/`ca.crt`. Check the cluster's status conditions. |
...
| Plaintext client suddenly can't connect | `required: true` removed ports `9000`/`8123`. Switch the client to `9440`/`8443`, or set `required: false` to keep insecure ports open during migration. |
